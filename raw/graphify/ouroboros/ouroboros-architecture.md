# Graph Report - /Users/rahilsinghi/Desktop/ouroboros  (2026-04-17)

## Corpus Check
- Corpus is ~49,999 words - fits in a single context window. You may not need a graph.

## Summary
- 653 nodes · 1390 edges · 22 communities detected
...
Cohesion: 0.04
Nodes (50): Terminal dashboard for scoreboard and ledger visualization., Render a compact summary of ledger entries., Render a scoreboard snapshot as an ASCII bar chart., render_ledger_summary(), render_scoreboard_ascii(), Compare before/after scoreboard snapshots and decide merge or rollback., ImplementResult, Implementer agent — writes code changes in a sandboxed worktree. (+42 more)

### Community 1 - "Community 1"
Cohesion: 0.07
Nodes (45): load_config(), OuroborosConfig, Configuration loading and validation for Ouroboros., Enum, EvalDecision, EvaluatorAgent, Evaluator agent — runs scoreboard and makes merge/rollback decisions., SandboxExecutor (+37 more)
...
Cohesion: 0.06
Nodes (48): _check_bloat(), MetaAgent, MetaResult, Meta-Agent — outer loop that evolves agent prompts., Result of a meta-agent cycle., Select the agent that causes the most failures based on telemetry., Check if mutation exceeds the bloat limit., Outer loop that evolves agent prompts based on telemetry. (+40 more)

### Community 4 - "Community 4"
Cohesion: 0.05
Nodes (20): AgentResponse, BaseAgent, CostTracker, Base agent class with LLM call wrapper., Calculate USD cost from token counts and model name., Accumulates API costs across calls., Record a call's token usage. Returns the cost of this call., Call the LLM with system and user prompts. (+12 more)
...
Cohesion: 0.07
Nodes (16): Trace analysis — pattern detection across runs., Produce aggregate statistics from trace events., TraceAnalyzer, TraceSummary, Collect trace events from claw-code CLI runs., TraceCollector, Analyze scoreboard and traces, return an ObservationReport., Persistent trace storage in JSONL format. (+8 more)

### Community 7 - "Community 7"
Cohesion: 0.09
Nodes (8): Safe rollback — clean up worktree and branch, never crash., Attempt to remove a worktree and its branch. Never raises., safe_rollback(), TestSafeRollback, TestWorktreeManager, Git worktree management for isolated improvement attempts., WorktreeInfo, WorktreeManager

### Community 8 - "Community 8"
Cohesion: 0.08
Nodes (11): PromptStore, Versioned prompt storage with atomic symlink swaps., Count tokens (words) in a prompt version., Manage versioned prompt files for each agent., Write v1.md and current.md for each agent from defaults (first run)., Load the current prompt for an agent. Falls back to hardcoded default., Return the highest version number for an agent., Write a new prompt version file. Returns the version number. (+3 more)
...
- **Thin community `Community 20`** (2 nodes): `__main__.py`, `Entry point for python -m ouroboros.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (2 nodes): `__init__.py`, `Ouroboros: Self-improving agent engine for claw-code.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `DimensionScore` connect `Community 2` to `Community 0`, `Community 3`, `Community 6`, `Community 11`, `Community 15`, `Community 17`?**
  _High betweenness centrality (0.198) - this node is a cross-community bridge._
- **Why does `BaseAgent` connect `Community 4` to `Community 0`, `Community 1`, `Community 3`, `Community 6`?**
  _High betweenness centrality (0.191) - this node is a cross-community bridge._
- **Why does `ImprovementLoop` connect `Community 1` to `Community 0`, `Community 3`, `Community 4`, `Community 7`, `Community 8`?**
